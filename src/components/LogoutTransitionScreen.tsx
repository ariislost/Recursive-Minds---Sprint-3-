...
interface LogoutTransitionScreenProps {
  onComplete: () => void;
  minDurationMs?: number;
}

export const LogoutTransitionScreen: React.FC<LogoutTransitionScreenProps> = ({ 
  onComplete, 
  minDurationMs = 1800 
}) => {
      ...
      {!isFinished && (
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none z-0">
            ...
            />
            {/* Fog Overlays */}
            <div className="absolute inset-0 bg-[#081018]/50 backdrop-blur-[1px]" />
            <div className="absolute inset-0 bg-radial-gradient from-transparent via-[#081018]/60 to-[#081018]" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#081018] via-transparent to-[#081018]/80" />
          </div>

          {/* Flying bills */}
          <FloatingBills />

          {/* SVG Viewport Rounded Progress Frame - Exactly matching the loading screen */}
          ...
          <div className="relative z-20 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-medium tracking-[0.25em] text-white/70 uppercase">
                ClaimSync • Locking Vault
              </span>
            </div>
            <div className="text-[11px] font-mono tracking-widest text-white/40 uppercase">
              {progress < 100 ? 'SECURING SESSION' : 'VAULT LOCKED'}
            </div>
          </div>

          {/* Bottom Left Minimalist Percentage Display */}
          <div className="relative z-20 flex flex-col justify-end mt-auto">
            ...
            <div className="text-xs sm:text-sm font-light text-white/50 tracking-wide mt-1 pl-1">
              Securing warranties, clearing cached credentials & locking session
            </div>
          </div>
        </motion.div>
      )}
